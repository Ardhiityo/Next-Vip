...
export default function ItemList({ products }: { products: Product[] }) {
  return (
    <section
      className="mt-5 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 place-items-center transition-all duration-300
   "
    >
      {products.map((product) => (
        <Link href={`/items/details/${product.id}`} key={product.id}>
          <div className="w-full max-w-sm bg-neutral-primary-soft p-6 border border-default rounded-base shadow-xs cursor-pointer">
            {product.image && (
              <Image
